...
&НаСервере
Процедура ТЧ_В_ДеревоНаСервере(ЗагрузкаДокумента)
	...
	Если НЕ Объект.ТЧ_Мониторинг.Количество() Тогда 
		Возврат;
	КонецЕсли;
	ТаблицаДопДанных = ПолучитьТЗЦЕН(Объект.ТЧ_Мониторинг.Выгрузить(,"Конкурент, Исполнитель, ЦенаКонкурента, Номенклатура"), Объект.Дата);
	Для Каждого строкаТЧ Из Объект.ТЧ_Мониторинг Цикл 
		...
		Если НЕ текКонкурент = строкаТЧ.Конкурент Тогда 
			...
			СтрУр2.ПрошлыйМониторингКомментарий = "тек: " + Формат(строкаТЧ.ДатаМониторинга, "ДФ=dd.MM.yyyy");
			Если СтрокиПоиска.Количество() Тогда 
				...
				Если ЗначениеЗаполнено(ДатаПрошлогоМониторинга) Тогда 
					СтрУр2.ПрошлыйМониторингКомментарий = СтрУр2.ПрошлыйМониторингКомментарий + ", пред: " + Формат(ДатаПрошлогоМониторинга, "ДФ=dd.MM.yyyy") + "(" + Окр(((Объект.Дата - ДатаПрошлогоМониторинга)/86400)) + " дн.)";
				КонецЕсли;
			КонецЕсли;
		...
		Если СтрокиПоиска.Количество() Тогда 
			...
			Если ЗначениеЗаполнено(СтрокаРасчета) Тогда 
				...
				Если Данные.ЦенаМагазинаНовая = 0 Тогда 
					ОшибкаФормулы = "Ошибка в формуле расчета новой цены!" 
				КонецЕсли;
			КонецЕсли;
			// Кириллов А.
			СтрУр3.НоваяЦена = ?(ЗначениеЗаполнено(строкаТЧ.НоваяЦена) ИЛИ ЗагрузкаДокумента, строкаТЧ.НоваяЦена, СтрУр3.РасчетнаяЦена);
			// Редакция 1Бит. Ошибка: если менеджер отказался от переоценки, он оставляет поле новой цены пустым, однако при загрузке оно заполнится значением по умолчанию.
			// СтрУр3.НоваяЦена = ?(ЗначениеЗаполнено(строкаТЧ.НоваяЦена),строкаТЧ.НоваяЦена,СтрУр3.РасчетнаяЦена);
		КонецЕсли;
		...
		СтрУр3.НоваяНаценка = ?(СтрУр3.ПрошлаяЦенаСебестоимость = 0 ИЛИ СтрУр3.НоваяЦена = 0, 0, (СтрУр3.НоваяЦена/СтрУр3.ПрошлаяЦенаСебестоимость-1)*100);
		СтрУр3.ПрошлыйМониторингКомментарий = ?(ЗначениеЗаполнено(строкаТЧ.Комментарий),строкаТЧ.Комментарий, ОшибкаФормулы); 
...
Функция ПолучитьТЗЦЕН(ТЧ_Документа, ДатаДокумента)
	...
	Запрос.Текст = 
		...
		|						ВТ_ЦеныИзМониторинга.Номенклатура
		|					ИЗ
		|						ВТ_ЦеныИзМониторинга КАК ВТ_ЦеныИзМониторинга)) КАК ЦеныНоменклатурыСрезПоследних
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныКонкурентов.СрезПоследних(
		|				&ДатаСрезаПрошлыхЦен,
		|				(Конкурент, Номенклатура) В
		|					(ВЫБРАТЬ
		|						ВТ_ЦеныИзМониторинга.Конкурент,
		|						ВТ_ЦеныИзМониторинга.Номенклатура
		|					ИЗ
		|						ВТ_ЦеныИзМониторинга КАК ВТ_ЦеныИзМониторинга)) КАК ЦеныКонкурентовСрезПоследних
		...
		|						ВТ_ЦеныИзМониторинга.Номенклатура
		|					ИЗ
		|						ВТ_ЦеныИзМониторинга КАК ВТ_ЦеныИзМониторинга)) КАК СебестоимостьНоменклатурыСрезПоследних
...
Процедура ДеревоИНТВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если текСтрока = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	Если текСтрока.Уровень<3 ИЛИ НЕ (Поле.Имя = "ДеревоИНТНоваяЦена" ИЛИ (Поле.Имя = "ДеревоИНТПрошлыйМониторингКомментарий" и текСтрока.СтатусПереоценки<>2)) Тогда 
	...
	Для Каждого СтрУр1 Из ДеревоЗначенийИзТЧ.Строки Цикл 
		...
		Для Каждого СтрУр2 Из СтрУр1.Строки Цикл
			...
			Для Каждого СтрУр3 Из СтрУр2.Строки Цикл
				СтрокаТЧ = ТекущийОбъект.ТЧ_Мониторинг[СтрУр3.НомерСтрокиТЧ-1];
				СтрокаТЧ.Комментарий = СтрУр3.ПрошлыйМониторингКомментарий;
...
Процедура ЗаполнитьТЧНаСервере()
	...
	Для Каждого СтрокаТЗ Из ТЗ Цикл 
		СтрокаТЧ = Объект.ТЧ_Мониторинг.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТЧ, СтрокаТЗ);
	КонецЦикла;
	
	ТЧ_В_ДеревоНаСервере(Ложь);
КонецПроцедуры
