...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	//ДеревоВ_ТЧ(ТекущийОбъект);//Ден  2017 12 01 ---
	...
	//ТЧ_В_ДеревоНаСервере();//Ден  2017 12 01 ---
КонецПроцедуры

&НаСервере
Процедура ТЧ_В_ДеревоНаСервере()
	ДеревоИНТЭлементы = ДеревоИНТ.ПолучитьЭлементы();
	ДеревоИНТЭлементы.Очистить(); //Ден  2017 12 01 ---
	...
	Для Каждого строкаТЧ Из Объект.Сопоставление Цикл 
		Если НЕ текТовар = строкаТЧ.Номенклатура Тогда 
			...
			СтрУр1 = ДеревоИНТЭлементы.Добавить();
			СтрУр1.Уровень = 1;
