...
Процедура ДеревоВ_ТЧ(ТекущийОбъект)
	...
	Для Каждого СтрУр1 Из ДеревоЗначенийИзТЧ.Строки Цикл 
		...
		Для Каждого СтрУр2 Из СтрУр1.Строки Цикл
			Если ЗначениеЗаполнено(СтрУр2.НомерСтрокиТЧ) Тогда 
				СтрокаТЧ = ТекущийОбъект.ТЧ_Сопоставление[СтрУр2.НомерСтрокиТЧ-1];
				
			Иначе 
				СтрокаТЧ = ТекущийОбъект.ТЧ_Сопоставление.Добавить();
	...
	ТекущийОбъект.ТЧ_Сопоставление.Сортировать("Номенклатура Возр");
...
Процедура ТЧ_В_ДеревоНаСервере()
	...
	Если НЕ Объект.ТЧ_Сопоставление.Количество() Тогда 
		Возврат;
	КонецЕсли;
	текТовар = Неопределено;
	ТаблицаЦенаКатегория = ПолучитьТЗЦЕН(Объект.ТЧ_Сопоставление.Выгрузить(,"Номенклатура").ВыгрузитьКолонку("Номенклатура"));
	Для Каждого строкаТЧ Из Объект.ТЧ_Сопоставление Цикл 
...
Процедура ДеревоИНТВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ДеревоИНТКомментарий" Тогда
		...
		Если ТекСтрока.Уровень = 2 Тогда 
			ПоказатьВводСтроки(Новый ОписаниеОповещения("ВводКомментария", ЭтаФорма, Новый Структура("ТолькоКомментарий",Истина)),ТекСтрока.КатегорияКомментарий, "Введите комментарий:");
		КонецЕсли;
...
&НаКлиенте
Процедура ДеревоИНТСопоставлениеКорректноПриИзменении(Элемент)
	//Текстрока = Элементы.ДеревоИНТ.ТекущиеДанные;
	//Если Текстрока.СопоставлениеКорректно = 0 Тогда 
	//	Элементы.ДеревоИНТ.ТекущийЭлемент = Элементы.ДеревоИНТКомментарий;
	//КонецЕсли;
КонецПроцедуры
...
Процедура Флажок(Команда)
	...
	ИначеЕсли ТекСтрока.СопоставлениеКорректно = 1 Тогда 
		ПоказатьВводСтроки(Новый ОписаниеОповещения("ВводКомментария", ЭтаФорма, Новый Структура),ТекСтрока.КатегорияКомментарий, "Введите комментарий:");
...
Функция ДоабвитьСтрокуВТЧ(МассивМагазиновНеВыводить, МассивКонкурентовНеВыводить, ТекМагазин)
	...
	Для Каждого СтрокаТЗ Из ТЗ Цикл 
		...
		СтрокаТЧ = Объект.ТЧ_Сопоставление.Добавить();
...
Процедура ЗаполнитьТЧНаСервере()
	СКД = РеквизитФормыВЗначение("Объект").ПолучитьМакет("Макет");
	Объект.ТЧ_Сопоставление.Очистить();
	...
	Объект.ТЧ_Сопоставление.Сортировать("Номенклатура Возр");
...
Процедура ВсеСопоставитьНаСервере()
	Для Каждого Стр Из Объект.ТЧ_Сопоставление Цикл
